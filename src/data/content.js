/**
 * Supporting content: reasons to choose the dealer, finance, process,
 * testimonials and USP stats.
 */

export const WHY_CHOOSE = [
  {
    icon: 'BadgeCheck',
    title: 'Certified NEXA Executive',
    text: 'Suriya Boopathy is an authorized Maruti Suzuki NEXA Senior Relationship Manager — genuine cars, transparent pricing and factory warranty.',
  },
  {
    icon: 'Car',
    title: 'Baleno Specialist',
    text: 'Baleno is my top-moving model — plus the full NEXA range: Fronx, Grand Vitara, Jimny, Invicto & XL6, ready for a test drive.',
  },
  {
    icon: 'Landmark',
    title: 'Punjab National Bank @ 7.75%',
    text: 'Loans up to 90% on-road funding at 7.75% interest through Punjab National Bank, with instant approval and every leading bank on tap.',
  },
  {
    icon: 'Rocket',
    title: 'Doorstep Test Drive',
    text: 'Book through me and I bring the Baleno, Fronx or any NEXA car to your doorstep — no showroom queue, no waiting.',
  },
  {
    icon: 'RefreshCw',
    title: '₹15,000 Exchange Bonus',
    text: 'This July, get a ₹15,000 exchange bonus on top of the best Maruti Suzuki True Value price for your old car.',
  },
  {
    icon: 'Users',
    title: '3 Years · 370+ Deliveries',
    text: 'Three focused years at NEXA and 370+ happy families delivered — priority allotment and personal attention on every booking.',
  },
  {
    icon: 'Wrench',
    title: 'Hassle-Free Documentation',
    text: 'I personally handle RTO, insurance and finance paperwork — you sign, I run around. Genuine spares and NEXA-grade service throughout.',
  },
  {
    icon: 'Sparkles',
    title: 'Priority Allotment',
    text: 'A premium NEXA lounge experience with priority allotment on booking — every customer treated like family.',
  },
]

export const HERO_STATS = [
  { value: '370+', label: 'Happy Families' },
  { value: '4.9★', label: 'Customer Rating' },
  { value: '3 Yrs', label: 'NEXA Experience' },
  { value: '24hr', label: 'Test Drive Booking' },
]

export const FINANCE_OPTIONS = [
  {
    icon: 'BadgeCheck',
    title: 'Consumer Offer · ₹15,000',
    text: 'Flat ₹15,000 consumer discount this July 2026 on Baleno and select NEXA models — booked directly through me.',
  },
  {
    icon: 'RefreshCw',
    title: 'Exchange Bonus · ₹15,000',
    text: 'Additional ₹15,000 exchange bonus over the best Maruti Suzuki True Value price for your old car.',
  },
  {
    icon: 'Landmark',
    title: 'PNB Finance @ 7.75%',
    text: 'Low-interest car loan at 7.75% through Punjab National Bank, up to 90% on-road funding with instant approval.',
  },
  {
    icon: 'ShieldCheck',
    title: 'Insurance Support',
    text: 'Hassle-free NEXA insurance with cashless claims and complete coverage guidance — I handle it end to end.',
  },
  {
    icon: 'Zap',
    title: 'Same-Day Approval',
    text: 'On-the-spot loan approval so you can book your Baleno the very same day and drive home stress-free.',
  },
]

export const PROCESS_STEPS = [
  {
    icon: 'Phone',
    title: 'Call or WhatsApp Me',
    text: 'Reach out on 73975 49735 or WhatsApp and share your favourite NEXA model — I take it from there.',
  },
  {
    icon: 'Car',
    title: 'Doorstep Test Drive',
    text: 'I bring the Baleno, Fronx or any NEXA car to your home or office — experience it on your roads, your time.',
  },
  {
    icon: 'Landmark',
    title: 'PNB Finance @ 7.75%',
    text: 'Pick the EMI and exchange plan, apply for Punjab National Bank finance at 7.75% and get same-day approval.',
  },
  {
    icon: 'FileCheck',
    title: 'Hassle-Free Documentation',
    text: 'I personally handle RTO, insurance and paperwork — quick, transparent and stress-free from start to finish.',
  },
  {
    icon: 'Gift',
    title: 'Joyful Delivery',
    text: 'Drive home your brand-new NEXA with a celebratory delivery experience — one of 370+ happy families.',
  },
]

export const TESTIMONIALS = [
  {
    name: 'Karthik Raja',
    location: 'Thanjavur',
    car: 'Suzuki Fronx Alpha Turbo',
    rating: 5,
    text: 'Best Fronx showroom in Thanjavur! The team gave me the best on-road price and my Turbo was delivered in just 10 days. Truly premium NEXA experience.',
  },
  {
    name: 'Priya Lakshmi',
    location: 'Kumbakonam',
    car: 'Baleno Alpha',
    rating: 5,
    text: 'I travelled from Kumbakonam and it was worth it. Transparent pricing, patient advisors and smooth finance. Highly recommend Suriya NEXA.',
  },
  {
    name: 'Mohammed Ismail',
    location: 'Pattukkottai',
    car: 'Grand Vitara Hybrid',
    rating: 5,
    text: 'Booked the Grand Vitara Strong Hybrid here. The exchange value for my old car was the highest I got anywhere in Thanjavur. Excellent service.',
  },
  {
    name: 'Suresh Kumar',
    location: 'Orathanadu',
    car: 'Suzuki Fronx Zeta',
    rating: 5,
    text: 'The mileage on my Fronx is fantastic and the buying process was completely hassle-free. Suriya NEXA is the most trusted NEXA dealer in Thanjavur.',
  },
  {
    name: 'Deepa Venkatesan',
    location: 'Papanasam',
    car: 'XL6 Zeta+',
    rating: 5,
    text: 'Our family loves the XL6. The advisors explained every variant clearly and never rushed us. Delivery experience was so special. Thank you team!',
  },
  {
    name: 'Arun Prasad',
    location: 'Thiruvidaimarudur',
    car: 'Jimny Alpha 4x4',
    rating: 5,
    text: 'Always wanted a Jimny and Suriya NEXA made it happen with easy EMI. Genuine, honest dealership. Five stars from me!',
  },
]

export const USP_BADGES = [
  'Authorized NEXA Dealer',
  'Best On-Road Price',
  'Instant Loan Approval',
  'Doorstep Test Drive',
  'Highest Exchange Value',
]
